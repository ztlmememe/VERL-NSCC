...
## Job name
#PBS -N dapo_qwen3_4b_2gpus_v2

## Merge standard output and error from PBS script
#PBS -j oe
#PBS -o /home/users/ntu/guoweia3/scratch/job_logs/create_dapo_qwen3_4b_2gpus_v2.%J.log
...
singularity exec --nv --cleanenv --no-home \
  --bind "$BIND_WORK":/workspace,"$BIND_DATA":/root \
  --pwd /workspace "$IMAGE" /bin/bash nscc/run_insidecon_dapo_qwen3_4b_2gpus_v2.sh
